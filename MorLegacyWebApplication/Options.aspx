...
<%'Page Title
  ...
  <title>
 <%if session("StoreName")<>"" then
  Response.write(NoQuotes(session("StoreName")))
 elseif session("RetailCustomerName")<>"" then
  Response.write(NoQuotes(session("RetailCustomerName")))
 else
  Response.Write(session("PowerUserName"))
 end if%>
 </title>
<%else%>
 <title>Account Login | Millions Of Records</title>
<%end if%>
...
function nocart(){
 alert('There are currently no items in your shopping cart.  If you wish, you can add items to your shopping cart and then save it to use at a later date.')
}
function recordslimit(){
 varElem=(isW3C) ? document.getElementById('ItemsPerPage'):document.all('ItemsPerPage');
 document.cookie="RecordsLimit="+varElem.value+"; path=/"
}
function submitform(){
 ...
 if (varElem.value.indexOf(".")==-1){
  alert('Please enter a valid Email Address. The Email address must contain a period (dot) character.')
  varElem.focus()
  return false
 }
...
function createPersonalAccount(){
 varElem=(isW3C) ? document.getElementById('WholesaleOrRetailTxt'):document.all('WholesaleOrRetailTxt');
 varElem.value="retail"
 document.NewCustomerForm.action="/country.aspx"
 document.NewCustomerForm.submit()
}
function createResellerAccount(){
 varElem = (isW3C) ? document.getElementById('WholesaleOrRetailTxt') : document.all('WholesaleOrRetailTxt');
 varElem.value = "wholesale"
 document.NewCustomerForm.action = "/country.aspx"
 document.NewCustomerForm.submit()
}
function txtForgotPasswordEmailKeyDown(evt){
...
function showContactUsDiv() {
 elemE = (isW3C) ? document.getElementById("divForgotEmail") : (document.all("divForgotEmail"));
 elemE.style.display = "none"
 elemE.style.visibility = "hidden"
 elemE = (isW3C) ? document.getElementById("divForgotPassword") : (document.all("divForgotPassword"));
 elemE.style.display = "none"
 elemE.style.visibility = "hidden"
 elem = (isW3C) ? document.getElementById("divContactUs") : (document.all("divContactUs"));
 elemTxt = (isW3C) ? document.getElementById("txtContactUsYourName") : (document.all("txtContactUsYourName"));
...
function showForgotPasswordDiv(){
 ...
 elemE.style.display="none"
 elemE.style.visibility="hidden"
 elemE = (isW3C) ? document.getElementById("divContactUs") : (document.all("divContactUs"));
 elemE.style.display = "none"
 elemE.style.visibility = "hidden"
 elem = (isW3C) ? document.getElementById("divForgotPassword") : (document.all("divForgotPassword"));
 elemTxt=(isW3C)?document.getElementById("txtForgotPasswordEmail"):(document.all("txtForgotPasswordEmail")); 
 elem.style.display="inline"
 elem.style.visibility="visible"
 elemTxt.focus()
}
function hideForgotPasswordDiv(){
 ...
 elemTxt=(isW3C)?document.getElementById("Pword"):(document.all("Pword")); 
 elemTxt.focus()
 elem.style.display="none"
 elem.style.visibility="hidden"
 elem2.style.display="none"
...
function txtForgotEmailPasswordKeyDown(evt){
 var unicode=evt.keyCode? evt.keyCode : evt.charCode
 if (unicode==13){
  forgotEmailSubmit()
 }
...
function showForgotEmailDiv(){
 elemP=(isW3C)?document.getElementById("divForgotPassword"):(document.all("divForgotPassword")); 
 elemP.style.display="none"
 elemP.style.visibility = "hidden"
 elemP = (isW3C) ? document.getElementById("divContactUs") : (document.all("divContactUs"));
 elemP.style.display = "none"
 elemP.style.visibility = "hidden"
 elem = (isW3C) ? document.getElementById("divForgotEmail") : (document.all("divForgotEmail"));
 elemTxt=(isW3C)?document.getElementById("txtForgotEmailPassword"):(document.all("txtForgotEmailPassword")); 
 elem.style.display="inline"
 elem.style.visibility="visible"
 elemTxt.focus()
}
function hideForgotEmailDiv(){
 ...
 elem3=(isW3C)?document.getElementById("divNoAccount"):(document.all("divNoAccount")); 
 elem4=(isW3C)?document.getElementById("divProcessingIcon"):(document.all("divProcessingIcon")); 
 elemTxt=(isW3C)?document.getElementById("LogInEmail"):(document.all("LogInEmail")); 
 elemTxt.focus()
 elem.style.display="none"
 elem.style.visibility="hidden"
 elem2.style.display="none"
...
function ProcessingTimerContactUs() {
 eLoadingImg = (isW3C) ? document.getElementById("imgContactUsLoadingImage") : document.all("imgContactUsLoadingImage");
 intX = intX + 1
 if (intX == 9) {
  intX = 1
 }
 eLoadingImg.src = "<%=AssetsPath()%>/loading-processing-" + intX + ".gif"
 tmrA = setTimeout("ProcessingTimerContactUs()", 100)
}
function EndProcessingTimerContactUs() {
 clearTimeout(tmrA)
 timer_is_on = 0
}
function ProcessingTimer(){
 eLoadingImg=(isW3C)?document.getElementById("imgLoadingImage"):document.all("imgLoadingImage");
 intX=intX+1
 if(intX==9){
  intX=1
 }
 eLoadingImg.src="<%=AssetsPath()%>/loading-processing-"+intX+".gif"
 tmrA=setTimeout("ProcessingTimer()",100)
}
function EndProcessingTimer(){
 clearTimeout(tmrA)
 timer_is_on=0
}
function ProcessingTimerForgotEmail(){
 eLoadingImg=(isW3C)?document.getElementById("imgForgotEmailLoadingImage"):document.all("imgForgotEmailLoadingImage");
 intX=intX+1
 if(intX==9){
  intX=1
 }
 eLoadingImg.src="<%=AssetsPath()%>/loading-processing-"+intX+".gif"
 tmrA=setTimeout("ProcessingTimerForgotEmail()",100)
}
function EndProcessingTimerForgotEmail(){
 clearTimeout(tmrA)
 timer_is_on=0
}

function txtSignInPasswordKeyDown(evt){
 var unicode=evt.keyCode? evt.keyCode : evt.charCode
 if (unicode==13){
  submitform()
 }
...
function contactUsSubmit() {
 ...
 if (elemYP.value == "") {
  elemYP.focus()
  alert('Please enter "Your Phone" before submitting.')
  return false
 }
 ...
 if (elemYE.value.indexOf("@") == -1) {
  elemYE.focus()
  alert('Please enter a valid "Your Email Address".  It may be missing the required "@" character.')
  return false
 }
 if (elemYE.value.indexOf(".") == -1) {
  elemYE.focus()
  alert('Please enter a valid "Your Email Address".  It may be missing the required dot "." character.')
  return false
 }
...
function forgotPasswordSubmit(){
 ...
 if (elem.value==""){
  elem.focus()
  alert('Please enter your "Email Address" so we can email your password to that address.')
  return false
 }
 if (elem.value.indexOf("@")==-1){
  elem.focus()
  alert('Please enter a valid "Email Address".  It may be missing the required "@" character.')
  return false
 }
 if (elem.value.indexOf(".")==-1){
  elem.focus()
  alert('Please enter a valid "Email Address".  It may be missing the required dot "." character.')
  return false
 }
 if (window.XMLHttpRequest){
  xmlhttp=new XMLHttpRequest()
...
function forgotEmailSubmit(){
 ...
 if (elem.value==""){
  elem.focus()
  alert('Please enter your "Password" so we can show your Email to allow you to sign in.')
  return false
 }
 if (window.XMLHttpRequest){
  xmlhttp=new XMLHttpRequest()
 ...
 if (!timer_is_on){
  ...
  elem4.style.visibility="visible"
  timer_is_on=1
  ProcessingTimerForgotEmail()
 }
 var ran=Math.floor((Math.random()*1000000)+1)
 xmlhttp.open("GET","/HTTPForgotEmail.aspx?P="+EscapeTotal(elem.value)+"&PF=Checkout"+"&ran="+ran,true)
 xmlhttp.onreadystatechange=function(){
  if (xmlhttp.readyState==4 && xmlhttp.status==200){
   EndProcessingTimerForgotEmail()
   elem4.style.display="none"
   elem4.style.visibility="hidden"
   if (xmlhttp.responseText=="none"){
    elem3.style.display="inline"
    elem3.style.visibility="visible"
   }else if (xmlhttp.responseText=="weak"){
...
function rememberMeSignIn(){
 elem=(isW3C)?document.getElementById("chkRememberMeSignIn"):(document.all("chkRememberMeSignIn")); 
 elem2=(isW3C)?document.getElementById("txtRememberMeSignIn"):(document.all("txtRememberMeSignIn")); 
...
function SD(z){
 varElem=(isW3C) ? document.getElementById(z):document.all(z);
 varElem.style.visibility='visible'
 varElem.style.display='inline'
}
function HD(z){
 sn10="a"
 varElem=(isW3C) ? document.getElementById(z):document.all(z);
...
<style type="text/css">
 ...
 input {font-family:Arial,Verdana,helvetica,sans-serif;font-size:13px;outline:none}
...
<body leftmargin="0" rightmargin="0" alink="000000" vlink="000000" bgcolor="000000">
<% ' Top Of Page %>
...
<p class="p-text-2"style="font-size:13px;color:#000000">We're sorry, the email recovery feature is not valid for the account linked to this password.
We recognize the password, however the password is not secure enough to prove positive authentication.
Therefore, if you can't remember your email then you must <a class="a-blue"style="font-size:13px;text-decoration:underline" href="mailto:ernieb12345@gmail.com?Subject=Sign%20In%20Problem" target="_top">contact us</a> us to sign in, or you can simply create a new account instead.</p>
</td></tr><tr><td width="55">
</td><td width="341"align="left"valign="top"style="padding-left:67px">
</td></tr><tr>
...
<p class="p-text-2">Enter the email address associated with your account, then click Submit. We'll instantly email your password so you can use it to sign in.
If you don't receive the email immediately then please check your spam folder.
Please add ernie@millionsofrecords.com to your email contacts to prevent spam filtering.</p>
</td><td width="50">
</td></tr></table>
</td></tr><tr><td>
<table width="560"align="left"style="text-align:left"border="0"cellpadding="0"cellspacing="0"frame="void">
...
<%'Sign In Error Messages and Sign Out User Messages
  ...
  <%Else%>
   ...
   <tr><td width="300">
    ...
    <p class="p-error">There is no account using that email address for sign-in, so please create a new account below.
    If you think that you already have an account with us, then please go to <a class="a-blue" onclick="showForgotPasswordDiv()"tabindex="-1"style="font-size:13px"href="#">Forgot&nbsp;Password</a> and enter a different email address that may have been used for sign-in. Note: It's OK to have more than one account with us, but each account must have a unique email address.
    </p>
    </td><td width="10">
    </td></tr></table>
   </td><td width="300">
  ...
  <%End If%>
<%End If%>
<%'Sign In From Email Broadcast Link Message
 ...
 <p class="p-error"style="font-size:14px">
 Please Sign In.<br/><br/>If you view items on our website without signing in, then you won't see your discounted (wholesale) prices.  Please sign in now and then the website will display your wholesale prices.  Thank you.
 </p>
 </td><td width="10">
 </td></tr></table>
 </td><td width="370">
...
<table width="900"style="text-align:left"border="0"cellpadding="0"cellspacing="0"frame="void">
  ...
  <table width="450"style="text-align:left;vertical-align:top"border="0"cellpadding="0"cellspacing="0"frame="void">
   ...
   <%Else%>
    <script language="javascript"type="text/javascript">
    document.SignInForm.Pword.focus()
    </script>
   <%end if%>
   </td></tr><tr><td width="65">
